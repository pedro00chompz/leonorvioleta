        ...
        Promise.all([
            ...
            const mergedData = [...page1Data, ...(page2Data.length > 0 ? page2Data : [])];
                    const printDataArray = mergedData
                        .filter(product => product.acf.display_in_sections.includes('canvas'))
                        .map(product => ({
    ...
    return (
        <div className="container-fluid printShopContainer">
            {printDataArray.map((printData, index) => (
                ...
                >
                    <div
                        className="order-2 order-md-1 col-12 col-md-3 borderRight borderRight-sm printShopText largePadding largePadding-md"
                        style={{fontSize: "1rem"}}>
                        <PrintInfo printData={printData}/>
                    </div>
                    <div
                        className="order-1 order-md-2 col-12 col-md-9 largePadding largePadding-md smallPadding-md forceBottomPadding">
